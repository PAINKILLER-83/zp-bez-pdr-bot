...
async def init_db():
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS users(
            user_id INTEGER PRIMARY KEY, trust INT DEFAULT 0, last_reset INT DEFAULT 0, hourly_count INT DEFAULT 0)""")
        await db.execute("""CREATE TABLE IF NOT EXISTS inbox(
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, caption TEXT,
            media_file_id TEXT, media_type TEXT, category TEXT, ts INT)""")
        await db.commit()
...
async def handle_category(update, context):
    ...
    uid = q.from_user.id
    async with aiosqlite.connect("bot.db") as db:
        cur = await db.execute("SELECT id,caption,media_file_id,media_type FROM inbox WHERE user_id=? AND category='' ORDER BY id DESC LIMIT 1", (uid,))
    ...
    text = f"🚗 Порушення ПДР у Запоріжжі\n🗂 Категорія: {category}\n📋 {PDR_MAP.get(category, '')}\n\n{caption}"
    if mtype == "photo":
